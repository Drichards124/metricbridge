...
required check on the `main` ruleset, Dependabot for Actions and uv, gate commands recorded in
`CLAUDE.md`. The spike stays read-only for reference.
...
catalogs (storefront, subscriptions, inventory, marketplace) as native YAML. Docker Compose for
Postgres and ClickHouse. Hand-written reference SQL per case. Parity matrix and divergence catalog

...
Three PRs: **1.7a** the harness and DuckDB, **1.7b** Postgres, **1.7c** ClickHouse. Each case is one
YAML file: a `query_metric` request and its expected outcome — rows from reference SQL written by
hand from the metric's definition, never from the compiler, or a refusal code. Reference SQL is
written once in DuckDB SQL and translated per engine by sqlglot, with a hand-written override only
where the translation is wrong; the normaliser converts types only. Corpus: the storefront catalog
(orders, subscriptions, inventory, shipments) on a larger deterministic seed; TPC-H (lineitem,
orders, part, supplier, customer, nation; one-hop joins) generated on demand by DuckDB's `tpch`
extension into a gitignored cache — SF0.1 in PR CI, SF1 nightly; and a marketplace catalog of
fan-trap and chasm-trap shapes. `local-data-warehouses/` holds one Docker Compose setup per engine,
run through `make test-<engine>` targets (1.7b, 1.7c). Parity matrix and divergence catalog
generated into `docs/conformance/` by `make conformance-docs`; the gates fail while they are stale.
**Verify:** E4 and E7. A divergence closes by fixing the compiler or adding a documented
normalisation rule with its own test; until then its case carries a `known_divergence` (D17).
The fan-trap and chasm-trap shapes — two facts joined through a shared dimension — must be refused
or computed correctly, never quietly fanned out (D11).
...
- **D17 · Conformance divergences (14 Sep 2026).** _Decided:_ a divergence that awaits its fix stays in the corpus: its case names the entry in `docs/failure-modes.md` as `known_divergence`, keeps the reference as computed, and must still differ, so the mark fails the moment the gap closes. An engine is _supported_ while every case on it passes or is such a recorded divergence; any unrecorded failure labels it experimental.
- **D18 · 1.7a fix decisions (15 Sep 2026).** _Decided:_ four questions raised by fixing the three divergences 1.7a found. (a) **Grain-to-date at a finer grain** — a metric declaring `grain_to_date` asked at a grain finer than that period — counts from the start of its calendar period, so month-to-date by day totals from the first of that day's month. A week crossing a month boundary reads as month-to-date on the week's *last* day, which is the rule the month grain already uses, so one rule holds at every grain; the days that week covers in the previous month are not counted. Rejected: counting from the month the week begins in, which makes a week's value depend on where it started rather than on what was true when it ended. (b) **Cuts through a join** are withheld only when the joined model has *both* measures and a partition dimension, which is exactly what the guardrail refuses — it bounds only models with measures, so a partitioned table without any is bounded by the fact it joins to and keeps its cuts. The broader "any partitioned model" rule would hide answerable cuts. (c) **When a fix closes the divergence a case was marked for but the case still differs for another reason**, the case is relabelled to the entry it now demonstrates and a second case is added covering the fixed behaviour with no mark. Keeping the marked case is deliberate: corpus cases run on every engine, so it is what will catch an engine-specific variation in 1.7b and 1.7c. (d) **Before the first release**, a fix to unreleased behaviour amends its `### Added` bullet rather than adding a `### Fixed` entry: no user saw the defect, so a repair note would be noise.
...
- 14 Sep 2026 — 1.7 design agreed: three PRs (1.7a harness + DuckDB, 1.7b Postgres, 1.7c ClickHouse); reference SQL written once in DuckDB SQL and sqlglot-translated per engine; TPC-H generated on demand by DuckDB's `tpch` extension, SF0.1 in PR CI and SF1 nightly (a minimal nightly workflow, extended by 1.8); engines run through `make test-<engine>` Docker Compose targets and each engine job becomes a required check as its PR lands; fan and chasm traps assert refusal. D17 added. 1.7a's corpus found three divergences, recorded in `docs/failure-modes.md`: a snapshot keeps one row on its chosen day, and a grain-to-date window starts at the period (both to fix PRs); and a cut through a joined model with its own partition is offered but refused by the guardrail (to stop being offered). Fan and chasm traps confirmed refused on every route.
- 15 Sep 2026 — all three of 1.7a's divergences fixed and merged: a snapshot now keeps every row on its chosen day (#36), cuts through a join to a partitioned model are no longer offered (#37), and a grain-to-date window starts at its calendar period (#38). D18 added, recording the four questions those fixes raised. DuckDB parity: 73 cases, 71 passing, 2 recorded divergences — both cases now sit under a single entry, anchor dropping in inactive periods, which waits on time spines in Phase 2. In `docs/failure-modes.md` the snapshot and grain-to-date rows move to guarded and the offered-but-refused cut moves to refused.

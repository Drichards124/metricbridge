...
| Grain-to-date window starts at the period | Month-to-date asked by day returns each day's own revenue: the window starts at the day, not at the first of its month | guarded | the window starts at `DATE_TRUNC(grain_to_date, …)` whenever the requested grain is finer (`tests/test_compiler.py::TestCumulativeAgainstDuckDB::test_grain_to_date_at_a_finer_grain_counts_from_the_start_of_the_month`); conformance case `revenue_month_to_date_by_day_with_no_gaps` agrees value for value |

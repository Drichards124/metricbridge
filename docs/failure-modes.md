...
| Inner join to a dimension | Rows with a null key vanish, shrinking the denominator | guarded | joins are always LEFT and each reports `<entity>__null_key_rows` (`tests/test_compiler.py` golden) |
...
| **Anchor dropping in inactive periods** | A period with no base rows never becomes an anchor, so its *trailing* total vanishes instead of rolling over: a December with no sales loses the preceding eleven months of trailing revenue, rather than reporting them | unguarded | deliberate until time spines (Phase 2). `CompiledQuery.expected_periods` lists every anchor the range should produce so execution can report the missing ones; demonstrated by `tests/test_compiler.py::TestCumulativeAgainstDuckDB::test_an_inactive_period_drops_its_anchor` |

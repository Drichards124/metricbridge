...
| Null join keys in ratio and cumulative answers | Rows with an empty key land in a `NULL` cut, and nothing says how many | unguarded | these shapes do not count null keys yet and answer `null_key_rows: null`, never `{}` (`tests/test_engine.py::TestAnswers::test_shapes_that_do_not_count_null_keys_say_so`) |
